...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	if (value != expo_value){  // don't repeat
		int it = (value & 0xffff);
		int again = (value & 0xffff0000) >> 16;
		struct again_lut *val_lut = sensor_again_lut;

		if (again >= 0) {  
			printk("    %s   sensor_set_expo: again = [%x]!\n", SENSOR_NAME, again);
		...
		/*integration time*/
		if (it > sensor_attr.max_integration_time) it = sensor_attr.max_integration_time;
		if (it > 2) {
			printk("    %s   sensor_set_expo: it = [%x]!\n", SENSOR_NAME, it);
...
{
	int ret = 0;
	printk("    %s sensor_set_integration_time: it = [%x]!\n", SENSOR_NAME, value);
	if (value > sensor_attr.max_integration_time) value = sensor_attr.max_integration_time;
...
static int sensor_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	if (value >= 0) {
		printk("    %s sensor_set_analog_gain: again = [%x]!\n", SENSOR_NAME, value);
...
{
	int ret;
	if (value != logic_value) {
		if (value > 0) {
			if (value <= 0x20) {
				//ret = sensor_set_analog_gain(sd,value);
			} else if ((value <= 0x10000) && (value > 0x100)) {
				//ret = sensor_set_integration_time(sd, value);
			} else if ((value <= 0x200000) && (value > 0x10000)) {
				ret = sensor_set_expo(sd, value);
			}
			logic_value = value;
		} else if ((value >> 12) == 0x82029) {
			int fps = (value & 0x00000fff) >> 8;
			//ret = sensor_set_fps(sd, fps);
		}
	}
        return 0;
}
...
#if 0
static int sensor_set_hflip(struct tx_isp_subdev *sd, int enable)
{
	int ret = -1;
	unsigned char val = 0x0;
	unsigned char col_start = 0x0;
	...
	ret += sensor_write(sd, 0x0d15, val);
	ret += sensor_write(sd, 0x0190, row_start);
...
static int sensor_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	if (*(int*) arg >= 0) 
		printk("*** %s sensor_sensor_ops_ioctl: cmd = [0x%x] arg = [0x%x] \n", SENSOR_NAME, cmd, *(int*)arg);
//udelay(10000);
	switch (cmd) {
	case TX_ISP_EVENT_SENSOR_EXPO:	// 16
		if (arg)
//			ret = sensor_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:  // 5
		if (arg)
			ret = sensor_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:		// 7
		if (arg)
			ret = sensor_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:		// 9
		if (arg)
			ret = sensor_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:	// B
		if (arg)
			ret = sensor_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:	// D
		if (arg)
			ret = sensor_set_mode(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_PREPARE_CHANGE:	// E
		if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI) {
			ret = sensor_write_array(sd, sensor_stream_off_mipi);
		} else {
			ISP_ERROR("Don't support this Sensor Data interface\n");
		...
		break;
	case TX_ISP_EVENT_SENSOR_FINISH_CHANGE:		// F
		if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI) {
			ret = sensor_write_array(sd, sensor_stream_on_mipi);
		} else {
			ISP_ERROR("Don't support this Sensor Data interface\n");
		...
		break;
	case TX_ISP_EVENT_SENSOR_FPS:		// A
		if (arg)
			ret = sensor_set_fps(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:		// 10
		if (arg)
			ret = sensor_set_vflip(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_LOGIC:		// 15
//		if (arg)
