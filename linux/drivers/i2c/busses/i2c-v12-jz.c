...
#define I2C_DEBUG 1
#define CONFIG_I2C_DEBUG_INFO 1
...
#undef I2C_DUP_ACCESS  // with dual sensor setting no need to duplicate access anymore
...
#ifdef I2C_DUP_ACCESS
/* #### TW:  dup ctrl  */
	uint	*iomem;
	if (i2c->adap.nr == 0) {
		if (!((int) i2c->iomem & 0x00001000)) {
			iomem = i2c->iomem + offset + i2c_iomem_offset;
		} else {
			iomem = i2c->iomem + offset - i2c_iomem_offset;  // swapped i2c bus
		}
		ret = readl(iomem);
  #ifdef CONFIG_I2C_DEBUG_INFO
	printk("i2c:     R name = %s (%d), iomem = 0x%08x, ret = %d \n", i2c->adap.name, i2c->adap.nr, i2c->iomem + offset, ret);
	printk("i2c: dup-R name = %s (%d), iomem = 0x%08x, ret = %d \n", i2c->adap.name, i2c->adap.nr, iomem, ret);
  #endif
 
	}
#endif

	return readl(i2c->iomem + offset) + ret;
}
...
/* #### TW:  dup ctrl  */
	...
	if (i2c->adap.nr == 0) {
		...
		writel(value, iomem);
  #ifdef CONFIG_I2C_DEBUG_INFO
	printk("i2c:     W name = %s (%d), iomem = 0x%08x\n", i2c->adap.name, i2c->adap.nr, i2c->iomem + offset);
	printk("i2c: dup-W name = %s (%d), iomem = 0x%08x\n", i2c->adap.name, i2c->adap.nr, iomem);
  #endif
...
/* ### TW:  */
#ifdef I2C_DEBUG
	printk("i2c:  name = %s (%d), iomem = 0x%08x, irq = %d \n", i2c->adap.name, i2c->adap.nr, i2c->iomem, i2c->irq);
